  ...
  methods: {
    ...
    deleteSalary(data) {
      this.$confirm('此操作将永久删除该【' + data.name + '】, 是否继续?', '提示', {
        ...
        this.deleteRequest('/salary/' + data.id).then(resp => {
          if (resp) {
            this.initSalaries();
          }
        });
    ...
    nextStep() {
      if (this.activeItemIndex == 10) {
        if (this.salary.id) {
          this.putRequest('/salary/', this.salary).then(resp => {
            if (resp) {
              this.initSalaries();
              this.dialogVisible = false;
            }
          ...
          this.postRequest('/salary/', this.salary).then(resp => {
            if (resp) {
              this.initSalaries();
              this.dialogVisible = false;
            }
    ...
    initSalaries() {
      this.getRequest('/salary/').then(resp => {
        this.salaries = resp;
      })
    }
  }
